LANG_SELECTION = {
    "title": {
        "en": "Choose language",
        "lv": "Izvēlies valodu",
    },
    "prompt": {
        "en": "Select language",
        "lv": "Izvēlies valodu",
    },
    "error": {
        "en": "Error: choose 1 or 2.",
        "lv": "Kļūda: izvēlies 1 vai 2.",
    },
    "options": {
        "1": {"en": "English", "lv": "English"},
        "2": {"en": "Latviešu", "lv": "Latviešu"},
    },
}

UI = {
    "en": {
        "title": "Expense Tracker",
        "language_title": "Choose language",
        "language_prompt": "Select language",
        "language_options": {
            "1": "English",
            "2": "Latviešu",
        },
        "invalid_language": "Error: choose 1 or 2.",
        "menu_prompt": "Choose an action",
        "invalid_choice_prefix": "Error: choose",
        "or_word": "or",
        "goodbye": "Goodbye!",
        "commands": {
            "1": "Add expense",
            "2": "Show expenses",
            "3": "Filter by month",
            "4": "Summary by category",
            "5": "Delete expense",
            "6": "Export CSV",
            "7": "Exit",
        },
        "date_prompt": "Date (YYYY-MM-DD)",
        "date_error": "Error: enter a valid date in YYYY-MM-DD format.",
        "category_label": "Category:",
        "category_prompt": "Choose category",
        "category_error": "Error: choose a valid category number.",
        "amount_prompt": "Amount (EUR)",
        "amount_error": "Error: enter a positive number, for example 12.50.",
        "description_prompt": "Description",
        "added_label": "Added",
        "no_expenses": "No expenses found.",
        "table_headers": ["Date", "Amount", "Category", "Description"],
        "total_label": "Total",
        "records_label": "records",
        "available_months_label": "Available months",
        "choose_month_prompt": "Choose month",
        "no_months": "No months available.",
        "category_summary_label": "Summary by category",
        "delete_prompt": "Enter expense number to delete (0 to cancel)",
        "delete_success": "Deleted",
        "delete_cancelled": "Deletion cancelled",
        "delete_error": "Error: choose a valid expense number.",
        "delete_confirm_message": "Delete this expense",
        "delete_confirm_prompt": "Confirm deletion (y/n)",
        "yes_choice": "y",
        "expense_number_header": "No.",
        "export_prompt": "Enter file name",
        "export_default_filename": "expenses.csv",
        "export_success": "Exported",
    },
    "lv": {
        "title": "Izdevumu izsekotājs",
        "language_title": "Izvēlies valodu",
        "language_prompt": "Izvēlies valodu",
        "language_options": {
            "1": "English",
            "2": "Latviešu",
        },
        "invalid_language": "Kļūda: izvēlies 1 vai 2.",
        "menu_prompt": "Izvēlies darbību",
        "invalid_choice_prefix": "Kļūda: izvēlies",
        "or_word": "vai",
        "goodbye": "Uz redzēšanos!",
        "commands": {
            "1": "Pievienot izdevumu",
            "2": "Parādīt izdevumus",
            "3": "Filtrēt pēc mēneša",
            "4": "Kopsavilkums pa kategorijām",
            "5": "Dzēst izdevumu",
            "6": "Eksportēt CSV",
            "7": "Iziet",
        },
        "date_prompt": "Datums (YYYY-MM-DD)",
        "date_error": "Kļūda: ievadi datumu formātā YYYY-MM-DD.",
        "category_label": "Kategorija:",
        "category_prompt": "Izvēlies kategoriju",
        "category_error": "Kļūda: izvēlies derīgu kategorijas numuru.",
        "amount_prompt": "Summa (EUR)",
        "amount_error": "Kļūda: ievadi pozitīvu skaitli, piemēram, 12.50.",
        "description_prompt": "Apraksts",
        "added_label": "Pievienots",
        "no_expenses": "Nav izdevumu.",
        "table_headers": ["Datums", "Summa", "Kategorija", "Apraksts"],
        "total_label": "Kopā",
        "records_label": "ieraksti",
        "available_months_label": "Pieejamie mēneši",
        "choose_month_prompt": "Izvēlies mēnesi",
        "no_months": "Nav pieejamu mēnešu.",
        "category_summary_label": "Kopsavilkums pa kategorijām",
        "delete_prompt": "Ievadi dzēšamā izdevuma numuru (0, lai atceltu)",
        "delete_success": "Dzēsts",
        "delete_cancelled": "Dzēšana atcelta",
        "delete_error": "Kļūda: izvēlies derīgu izdevuma numuru.",
        "delete_confirm_message": "Dzēst šo izdevumu",
        "delete_confirm_prompt": "Apstiprini dzēšanu (j/n)",
        "yes_choice": "j",
        "expense_number_header": "Nr.",
        "export_prompt": "Ievadi faila nosaukumu",
        "export_default_filename": "expenses.csv",
        "export_success": "Eksportēts",
    },
}

CATEGORY_LABELS = {
    "en": {
        "food": "Food",
        "transport": "Transport",
        "entertainment": "Entertainment",
        "utilities": "Utilities",
        "health": "Health",
        "shopping": "Shopping",
        "other": "Other",
    },
    "lv": {
        "food": "Ēdiens",
        "transport": "Transports",
        "entertainment": "Izklaide",
        "utilities": "Komunālie maksājumi",
        "health": "Veselība",
        "shopping": "Iepirkšanās",
        "other": "Cits",
    },
}
